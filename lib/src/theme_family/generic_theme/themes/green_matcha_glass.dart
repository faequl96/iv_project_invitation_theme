import 'package:flutter/material.dart';
import 'package:gradient_borders/box_borders/gradient_box_border.dart';
import 'package:iv_project_invitation_theme/src/theme_family/generic_theme/generic_configs.dart';
import 'package:iv_project_invitation_theme/src/widgets/bubble_background.dart';
import 'package:iv_project_invitation_theme/src/widgets/particle_sphere.dart';
import 'package:quick_dev_sdk/quick_dev_sdk.dart';

class GreenMatchaGlass {
  GreenMatchaGlass._();

  static GenericConfigs get configs => GenericConfigs(
    tabConfig: GenericTabConfig(
      useGlassEffect: true,
      indicatorColor: Colors.green.shade50,
      backgroundColor: ColorUtil.darken(Colors.green.shade900, 50),
      titleActiveColor: Colors.white,
      titleInactiveColor: Colors.green.shade200,
      iconActiveColor: Colors.white,
      iconInactiveColor: Colors.green.shade200,
    ),
    globalBackgroundsBuilder: () => [
      BubbleBackground(
        backgroundColor: ColorUtil.darken(Colors.green.shade100, 20).withValues(alpha: .6),
        bubbleColor1: ColorUtil.darken(Colors.lightGreenAccent, 15),
        bubbleColor2: ColorUtil.darken(Colors.green.shade900, 40),
        bubbleColor3: ColorUtil.darken(Colors.green.shade700, 40),
        bubbleColor4: ColorUtil.darken(Colors.green.shade400, 5),
        bubbleColor5: ColorUtil.darken(Colors.lightGreenAccent, 10),
        noAnimate: false,
      ),
    ],
    particleSphere: .circle(
      circleParticleVariations: [
        CircleParticle(color: ColorUtil.darken(Colors.lightGreenAccent, 5)),
        const CircleParticle(color: Colors.white),
        CircleParticle(color: ColorUtil.darken(Colors.green.shade700, 40)),
      ],
    ),
    coverPageConfig: GenericCoverPageConfig(
      gradientBaseColor: ColorUtil.darken(Colors.green.shade700, 60),
      brideNameColor: Colors.green.shade200,
      groomNameColor: Colors.green.shade200,
      countdownBorderWidth: 1,
      countdownOddColor: Colors.white.withValues(alpha: .15),
      countdownEvenColor: Colors.white.withValues(alpha: .15),
      countdownOddBorderColor: Colors.green.shade200,
      countdownEvenBorderColor: Colors.green.shade200,
      countdownNumberColor: Colors.green.shade50,
      countdownUnitColor: Colors.green.shade50,
      useLightningEffectOnCountdown: true,
      firstArrowColor: Colors.green.shade100.withValues(alpha: .7),
      secondArrowColor: Colors.green.shade300.withValues(alpha: .5),
    ),
    firstPageConfig: GenericFirstPageConfig(
      useBackdropBlurOnScaffold: true,
      firstGradientBackgroundColor: ColorUtil.darken(Colors.green.shade700, 60),
      secondGradientBackgroundColor: ColorUtil.darken(
        Colors.green.shade700,
        60,
      ).withValues(alpha: .0),
      firstGradientScaffoldColor: Colors.white.withValues(alpha: .2),
      secondGradientScaffoldColor: Colors.white.withValues(alpha: .2),
      scaffoldBorder: GradientBoxBorder(
        width: 1.5,
        gradient: LinearGradient(
          begin: .topLeft,
          end: .bottomRight,
          colors: [
            Colors.white,
            Colors.white.withValues(alpha: .2),
            Colors.white,
            Colors.white.withValues(alpha: .2),
            Colors.white,
          ],
          transform: const GradientRotation(-0.2),
        ),
      ),
      useGlassEffectOnScaffold: true,
      glassEffectOpacity: .5,
      titlePageColor: Colors.white,
      openingTextColor: Colors.white,
      generalTextColor: ColorUtil.darken(Colors.green.shade700, 30),
      firstSubScaffoldColor: Colors.white.withValues(alpha: .4),
      firstSubScaffoldBorderColor: Colors.white.withValues(alpha: .7),
      firstSubScaffoldBorderWidth: 1,
      secondSubScaffoldColor: Colors.white.withValues(alpha: .3),
      secondSubScaffoldBorderColor: Colors.white.withValues(alpha: .7),
      secondSubScaffoldBorderWidth: 1,
    ),
    secondPageConfig: GenericSecondPageConfig(
      useBackdropBlurOnScaffold: true,
      firstGradientScaffoldColor: Colors.white.withValues(alpha: .2),
      secondGradientScaffoldColor: Colors.white.withValues(alpha: .3),
      scaffoldBorder: GradientBoxBorder(
        width: 1.5,
        gradient: LinearGradient(
          begin: .topLeft,
          end: .bottomRight,
          colors: [
            Colors.white,
            Colors.white.withValues(alpha: .2),
            Colors.white,
            Colors.white.withValues(alpha: .2),
            Colors.white,
          ],
          transform: const GradientRotation(-0.2),
        ),
      ),
      useGlassEffectOnScaffold: true,
      glassEffectOpacity: .6,
      titlePageColor: ColorUtil.darken(Colors.green.shade900, 20),
      generalTextColor: ColorUtil.darken(Colors.green.shade500, 5),
      brideDividingBorderWidth: 1,
      brideImageBorderWidth: 1,
      brideDividingLineWidth: 1,
      brideDividingBorderColor: Colors.green.shade400,
      brideImageFrameColor: ColorUtil.darken(Colors.green.shade400, 5).withValues(alpha: .3),
      brideImageFrameBorderColor: ColorUtil.darken(Colors.green.shade400, 5).withValues(alpha: .8),
      brideDividingLineColor: Colors.green.shade400,
      brideNameTextColor: ColorUtil.darken(Colors.green.shade700),
      brideFatherNameTextColor: ColorUtil.darken(Colors.green.shade700),
      brideMotherNameTextColor: ColorUtil.darken(Colors.green.shade700),
      groomDividingBorderWidth: 1,
      groomImageBorderWidth: 1,
      groomDividingLineWidth: 1,
      groomDividingBorderColor: Colors.green.shade400,
      groomImageFrameColor: ColorUtil.darken(Colors.green.shade400, 5).withValues(alpha: .3),
      groomImageFrameBorderColor: ColorUtil.darken(Colors.green.shade400, 5).withValues(alpha: .8),
      groomDividingLineColor: Colors.green.shade400,
      groomNameTextColor: ColorUtil.darken(Colors.green.shade700),
      groomFatherNameTextColor: ColorUtil.darken(Colors.green.shade700),
      groomMotherNameTextColor: ColorUtil.darken(Colors.green.shade700),
    ),
    thirdPageConfig: GenericThirdPageConfig(
      useBackdropBlurOnScaffold: true,
      firstGradientBackgroundColor: ColorUtil.darken(
        Colors.green.shade700,
        60,
      ).withValues(alpha: .0),
      secondGradientBackgroundColor: ColorUtil.darken(Colors.green.shade700, 60),
      firstGradientScaffoldColor: Colors.white.withValues(alpha: .2),
      secondGradientScaffoldColor: Colors.white.withValues(alpha: .7),
      stopsGradientScaffoldColor: [.0, .5],
      scaffoldBorder: GradientBoxBorder(
        width: 1.5,
        gradient: LinearGradient(
          begin: .topLeft,
          end: .bottomRight,
          colors: [
            Colors.white,
            Colors.white.withValues(alpha: .2),
            Colors.white,
            Colors.white.withValues(alpha: .2),
            Colors.white,
          ],
          transform: const GradientRotation(-0.2),
        ),
      ),
      useGlassEffectOnScaffold: true,
      glassEffectOpacity: .5,
      titlePageColor: ColorUtil.darken(Colors.green.shade900, 20),
      contractTitleColor: ColorUtil.darken(Colors.green.shade700),
      contractBaseTextColor: ColorUtil.darken(Colors.green.shade700),
      receptionTitleColor: ColorUtil.darken(Colors.green.shade700),
      receptionBaseTextColor: ColorUtil.darken(Colors.green.shade700),
      dividingLineWidth: 1,
      dividingLineColor: Colors.green.shade300,
      contractCountdownBorderWidth: 1,
      contractCountdownOddColor: Colors.white.withValues(alpha: .3),
      contractCountdownEvenColor: Colors.white.withValues(alpha: .3),
      contractCountdownOddBorderColor: Colors.green.shade500,
      contractCountdownEvenBorderColor: Colors.green.shade500,
      contractCountdownNumberColor: ColorUtil.darken(Colors.green.shade700),
      contractCountdownUnitColor: ColorUtil.darken(Colors.green.shade700),
      receptionCountdownBorderWidth: 1,
      receptionCountdownOddColor: Colors.white.withValues(alpha: .4),
      receptionCountdownEvenColor: Colors.white.withValues(alpha: .4),
      receptionCountdownOddBorderColor: Colors.green.shade500,
      receptionCountdownEvenBorderColor: Colors.green.shade500,
      receptionCountdownNumberColor: ColorUtil.darken(Colors.green.shade700),
      receptionCountdownUnitColor: ColorUtil.darken(Colors.green.shade700),
      useLightningEffectOnCountdown: true,
    ),
    thirdDifferentLocationPageConfig: GenericThirdDifferentLocationPageConfig(
      useBackdropBlurOnScaffold: true,
      firstGradientBackgroundColor: ColorUtil.darken(
        Colors.green.shade700,
        60,
      ).withValues(alpha: .0),
      secondGradientBackgroundColor: ColorUtil.darken(Colors.green.shade700, 60),
      firstGradientScaffoldColor: Colors.white.withValues(alpha: .2),
      secondGradientScaffoldColor: Colors.white.withValues(alpha: .7),
      stopsGradientScaffoldColor: [.0, .5],
      scaffoldBorder: GradientBoxBorder(
        width: 1.5,
        gradient: LinearGradient(
          begin: .topLeft,
          end: .bottomRight,
          colors: [
            Colors.white,
            Colors.white.withValues(alpha: .2),
            Colors.white,
            Colors.white.withValues(alpha: .2),
            Colors.white,
          ],
          transform: const GradientRotation(-0.2),
        ),
      ),
      useGlassEffectOnScaffold: true,
      glassEffectOpacity: .5,
      titlePageColor: ColorUtil.darken(Colors.green.shade900, 20),
      dateTimeIconColor: ColorUtil.darken(Colors.green.shade700),
      dateTimeBaseTextColor: ColorUtil.darken(Colors.green.shade700),
      countdownBorderWidth: 1,
      countdownOddColor: Colors.white.withValues(alpha: .3),
      countdownEvenColor: Colors.white.withValues(alpha: .3),
      countdownOddBorderColor: Colors.green.shade500,
      countdownEvenBorderColor: Colors.green.shade500,
      countdownNumberColor: ColorUtil.darken(Colors.green.shade700),
      countdownUnitColor: ColorUtil.darken(Colors.green.shade700),
      useLightningEffectOnCountdown: false,
      dividingLineWidth: 1,
      dividingLineColor: ColorUtil.darken(Colors.green.shade500),
      placeIconColor: ColorUtil.darken(Colors.green.shade700),
      placeBaseTextColor: ColorUtil.darken(Colors.green.shade700),
      addressTextColor: ColorUtil.darken(Colors.green.shade700),
      mapsBorderColor: Colors.white.withValues(alpha: .7),
      getDirectionsButtonColor: ColorUtil.darken(Colors.green.shade500).withValues(alpha: .8),
      getDirectionsButtonLabelColor: Colors.white,
      getDirectionsButtonBorderWidth: 1.5,
      getDirectionsButtonBorderColor: Colors.white,
    ),
    fourthPageConfig: GenericFourthPageConfig(
      useBackdropBlurOnScaffold: true,
      firstGradientBackgroundColor: ColorUtil.darken(Colors.green.shade700, 60),
      secondGradientBackgroundColor: ColorUtil.darken(
        Colors.green.shade700,
        60,
      ).withValues(alpha: .0),
      firstGradientScaffoldColor: Colors.white.withValues(alpha: .2),
      secondGradientScaffoldColor: Colors.white.withValues(alpha: .4),
      scaffoldBorder: GradientBoxBorder(
        width: 1.5,
        gradient: LinearGradient(
          begin: .topLeft,
          end: .bottomRight,
          colors: [
            Colors.white,
            Colors.white.withValues(alpha: .2),
            Colors.white,
            Colors.white.withValues(alpha: .2),
            Colors.white,
          ],
          transform: const GradientRotation(-0.2),
        ),
      ),
      useGlassEffectOnScaffold: true,
      glassEffectOpacity: .5,
      titlePageColor: Colors.white,
      placeIconColor: Colors.white,
      placeTextColor: Colors.white,
      addressTextColor: Colors.white,
      dividingLineWidth: 1,
      dividingLineColor: Colors.white,
      mapsBorderColor: Colors.white.withValues(alpha: .7),
      getDirectionsButtonColor: ColorUtil.darken(Colors.green.shade500).withValues(alpha: .8),
      getDirectionsButtonLabelColor: Colors.white,
      getDirectionsButtonBorderWidth: 1.5,
      getDirectionsButtonBorderColor: Colors.white,
    ),
    fourthDifferentLocationPageConfig: GenericFourthDifferentLocationPageConfig(
      useBackdropBlurOnScaffold: true,
      firstGradientBackgroundColor: ColorUtil.darken(Colors.green.shade700, 60),
      secondGradientBackgroundColor: ColorUtil.darken(
        Colors.green.shade700,
        60,
      ).withValues(alpha: .0),
      firstGradientScaffoldColor: Colors.white.withValues(alpha: .2),
      secondGradientScaffoldColor: Colors.white.withValues(alpha: .4),
      scaffoldBorder: GradientBoxBorder(
        width: 1.5,
        gradient: LinearGradient(
          begin: .topLeft,
          end: .bottomRight,
          colors: [
            Colors.white,
            Colors.white.withValues(alpha: .2),
            Colors.white,
            Colors.white.withValues(alpha: .2),
            Colors.white,
          ],
          transform: const GradientRotation(-0.2),
        ),
      ),
      useGlassEffectOnScaffold: true,
      glassEffectOpacity: .5,
      titlePageColor: Colors.white,
      dateTimeIconColor: Colors.white,
      dateTimeBaseTextColor: Colors.white,
      countdownBorderWidth: 1,
      countdownOddColor: ColorUtil.lighten(Colors.green.shade50, 40).withValues(alpha: .6),
      countdownEvenColor: ColorUtil.lighten(Colors.green.shade50, 40).withValues(alpha: .6),
      countdownOddBorderColor: Colors.white,
      countdownEvenBorderColor: Colors.white,
      countdownNumberColor: ColorUtil.darken(Colors.green.shade700),
      countdownUnitColor: ColorUtil.darken(Colors.green.shade700),
      useLightningEffectOnCountdown: false,
      dividingLineWidth: 1,
      dividingLineColor: Colors.white,
      placeIconColor: Colors.white,
      placeBaseTextColor: Colors.white,
      addressTextColor: Colors.white,
      mapsBorderColor: Colors.white.withValues(alpha: .7),
      getDirectionsButtonColor: ColorUtil.darken(Colors.green.shade500).withValues(alpha: .8),
      getDirectionsButtonLabelColor: Colors.white,
      getDirectionsButtonBorderWidth: 1.5,
      getDirectionsButtonBorderColor: Colors.white,
    ),
    fifthPageConfig: GenericFifthPageConfig(
      useBackdropBlurOnScaffold: true,
      firstGradientScaffoldColor: Colors.white.withValues(alpha: .2),
      secondGradientScaffoldColor: Colors.white.withValues(alpha: .4),
      scaffoldBorder: GradientBoxBorder(
        width: 1.5,
        gradient: LinearGradient(
          begin: .topLeft,
          end: .bottomRight,
          colors: [
            Colors.white,
            Colors.white.withValues(alpha: .2),
            Colors.white,
            Colors.white.withValues(alpha: .2),
            Colors.white,
          ],
          transform: const GradientRotation(-0.2),
        ),
      ),
      useGlassEffectOnScaffold: true,
      glassEffectOpacity: .6,
      titlePageColor: ColorUtil.darken(Colors.green.shade900, 20),
      dividingLineWidth: 1,
      dividingVerticalLineColor: ColorUtil.darken(Colors.green.shade200),
      dividingHorizontalLineColor: ColorUtil.darken(Colors.green.shade200),
      seeMoreButtonColor: ColorUtil.darken(Colors.green.shade500).withValues(alpha: .8),
      seeMoreButtonLabelColor: Colors.white,
      seeMoreButtonBorderWidth: 1.5,
      seeMoreButtonBorderColor: Colors.white,
      bottomSheetHandleColor: Colors.white,
      bottomSheetContentScaffoldColor: Colors.white.withValues(alpha: .8),
      bottomSheetBackgroundColor: ColorUtil.darken(Colors.green.shade900, 50).withValues(alpha: .8),
      bottomSheetCloseIconColor: Colors.white,
      bottomSheetOnHoverCloseIconColor: Colors.green.shade400,
    ),
    sixthPageConfig: GenericSixthPageConfig(
      useBackdropBlurOnScaffold: true,
      firstGradientBackgroundColor: ColorUtil.darken(
        Colors.green.shade700,
        60,
      ).withValues(alpha: 0),
      secondGradientBackgroundColor: ColorUtil.darken(Colors.green.shade700, 60),
      firstGradientScaffoldColor: Colors.white.withValues(alpha: .2),
      secondGradientScaffoldColor: Colors.white.withValues(alpha: .5),
      stopsGradientScaffoldColor: [.0, .5],
      scaffoldBorder: GradientBoxBorder(
        width: 1.5,
        gradient: LinearGradient(
          begin: .topLeft,
          end: .bottomRight,
          colors: [
            Colors.white,
            Colors.white.withValues(alpha: .2),
            Colors.white,
            Colors.white.withValues(alpha: .2),
            Colors.white,
          ],
          transform: const GradientRotation(-0.2),
        ),
      ),
      useGlassEffectOnScaffold: true,
      glassEffectOpacity: .5,
      titlePageColor: ColorUtil.darken(Colors.green.shade900, 20),
      introductionTextColor: ColorUtil.darken(Colors.green.shade700),
      introductionColor: Colors.white.withValues(alpha: .3),
      introductionBorderColor: Colors.white,
      introductionBorderWidth: 1,
      bankColor: Colors.white.withValues(alpha: .4),
      bankBorderColor: Colors.white,
      bankBorderWidth: 1,
      bankTextColor: ColorUtil.darken(Colors.green.shade700),
      bankCopyBaseColor: ColorUtil.darken(Colors.green.shade500),
    ),
    seventhPageConfig: GenericSeventhPageConfig(
      useBackdropBlurOnScaffold: true,
      firstGradientBackgroundColor: ColorUtil.darken(Colors.green.shade700, 60),
      secondGradientBackgroundColor: ColorUtil.darken(
        Colors.green.shade700,
        60,
      ).withValues(alpha: 0),
      firstGradientScaffoldColor: Colors.white.withValues(alpha: .5),
      secondGradientScaffoldColor: Colors.white.withValues(alpha: .2),
      stopsGradientScaffoldColor: [.5, 1],
      scaffoldBorder: GradientBoxBorder(
        width: 1.5,
        gradient: LinearGradient(
          begin: .topLeft,
          end: .bottomRight,
          colors: [
            Colors.white,
            Colors.white.withValues(alpha: .2),
            Colors.white,
            Colors.white.withValues(alpha: .2),
            Colors.white,
          ],
          transform: const GradientRotation(-0.2),
        ),
      ),
      useGlassEffectOnScaffold: true,
      glassEffectOpacity: .5,
      titlePageColor: Colors.white,
      fieldTextColor: Colors.white,
      fieldLabelColor: Colors.white,
      fieldFillColor: Colors.green.shade700.withValues(alpha: .2),
      fieldBorderColor: Colors.white,
      fieldSplashColor: Colors.white.withValues(alpha: .5),
      overlayColor: Colors.white.withValues(alpha: .9),
      overlayBorderColor: Colors.white,
      dropdownItemSelectedColor: Colors.green.shade200,
      dropdownItemTextColor: Colors.green.shade700,
      dropdownItemSelectedTextColor: Colors.white,
      // dropdownItemHoveredColor: Colors.grey.shade100,
      // dropdownItemSplashColor: Colors.grey.shade100,
      submitButtonColor: ColorUtil.darken(Colors.green.shade500).withValues(alpha: .8),
      submitButtonLabelColor: Colors.white,
      submitButtonBorderWidth: 1.5,
      submitButtonBorderColor: Colors.white,
      rsvpBackgroundColor: Colors.white.withValues(alpha: .8),
      rsvpBorderColor: Colors.white,
      rsvpNameTextColor: ColorUtil.darken(Colors.green.shade700),
      rsvpTimeAgoTextColor: Colors.grey.shade800,
      rsvpNameInstanceTextColor: Colors.grey.shade600,
      rsvpAttendanceTextColorLighten: 20,
      rsvpMessageTextColor: ColorUtil.darken(Colors.green.shade600),
      seeMoreButtonColor: ColorUtil.darken(Colors.green.shade500).withValues(alpha: .8),
      seeMoreButtonLabelColor: Colors.white,
      seeMoreButtonBorderWidth: 1.5,
      seeMoreButtonBorderColor: Colors.white,
      bottomSheetHandleColor: Colors.white,
      bottomSheetContentScaffoldColor: Colors.white.withValues(alpha: .9),
      bottomSheetBackgroundColor: ColorUtil.darken(Colors.green.shade900, 50).withValues(alpha: .8),
      bottomSheetCloseIconColor: Colors.white,
      bottomSheetOnHoverCloseIconColor: Colors.green.shade300,
    ),
    eighthPageConfig: GenericEighthPageConfig(
      useBackdropBlurOnScaffold: true,
      firstGradientScaffoldColor: Colors.white.withValues(alpha: .3),
      secondGradientScaffoldColor: Colors.white.withValues(alpha: .3),
      scaffoldBorder: GradientBoxBorder(
        width: 1.5,
        gradient: LinearGradient(
          begin: .topLeft,
          end: .bottomRight,
          colors: [
            Colors.white,
            Colors.white.withValues(alpha: .2),
            Colors.white,
            Colors.white.withValues(alpha: .2),
            Colors.white,
          ],
          transform: const GradientRotation(-0.2),
        ),
      ),
      useGlassEffectOnScaffold: true,
      glassEffectOpacity: .6,
      titlePageColor: ColorUtil.darken(Colors.green.shade900, 20),
      closingTextColor: ColorUtil.darken(Colors.green.shade700),
      brideGroomNameColor: ColorUtil.darken(Colors.green.shade700),
      brandBackgroundColor: Colors.grey.shade200.withValues(alpha: .7),
      brandTextColor: Colors.grey.shade600,
    ),
  );

  static ThemeData? get themeData => null;
}
